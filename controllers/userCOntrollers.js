...
exports.registerUser = async (req, res) => {
    try {
        const { fullName, email, username, password, gender, confirmPassword } = req.body;
        if (!fullName || !email || !username || !password || !gender || !confirmPassword) {
...
exports.lopgoutUser = async (req, res) => {
    try {
        return res.status(200).cookie("token", "", { maxAge: 0, httpOnly: true, sameSite: "strict" }).json({ message: "Logout successful" });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ message: "Internal server error" });
    }
